...
import { newRegister } from "./productos-new.js";
import { editRegister } from "./productos-new.js";
...
export async function Productos(){
    ...
    res.forEach(element => {
      element.action = "<div class='btn-group'><a class='btn btn-warning btn-sm mr-1 rounded-circle btnEditarProducto'  href='#/editProducto' data-idProductoo='"+ element.id +"'> <i class='fas fa-pencil-alt'></i></a><a class='btn btn-danger btn-sm rounded-circle removeItem btnBorrarProducto'href='#/delProducto' data-idProducto='"+ element.id +"'><i class='fas fa-trash'></i></a></div>";
...
function editar(){
    let id = parseInt(this.getAttribute('data-idProducto'), 10);
    editRegister(id);
}
...
function llenarTabla(res){ 
    dtable = new DataTable('#productosTable', {
        responsive:true,
        data : res,
        columns: [
            { data: 'id' },    
